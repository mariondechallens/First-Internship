...
predictFUN<-function(object,newdata,id){   ## prediction sur un nouveau jeu de donnees pour les modeles regsubsets
...
plot_model<-function(model,data,title){
  ...
  legend("bottomleft",
         legend=c("Real values","Predicted values"),
         fill=c("red","blue"),
         border=c("red","blue"))
  dev.off()
  return(model)
...
for_back<-function(elimination, nbvar,data){
  ...
  jpeg_SOA(paste0(regDir,"Models' root MSE ",elimination,".jpeg"))
  plot(sqrt(mse),
       ylab= "Root MSE des modeles",
       main=paste0("Modeles ",elimination),
       pch=19,
       xlab="nb of variables",
       ylim=c(min(sqrt(mse),sqrt(forw$rss[-1]/round(0.7*nrow(table)))),max(sqrt(mse),sqrt(forw$rss[-1]/round(0.7*nrow(table))))),
       type='b')
  ...
  jpeg_SOA(paste0(regDir,"Models' R2 ",elimination,".jpeg"))
  plot(summary(forw)$rsq,
       ylab="R2 and R2 adjusted", 
       main=paste0("R2 for ",elimination),
       xlab="nb of variables",
       pch=19,
       col="blue",
       type="b")
